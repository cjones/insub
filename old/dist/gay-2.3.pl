...
%IRSSI = (
	...
	changed		=> 'Wed Jul 16 14:29:33 PDT 2003',
	version		=> $VERSION,
);
...
sub jivesay   { process("j",   @_) }    # say something in jive
...
sub process {
	...
	while (my $arg = shift(@args)) {
		...
		if ($arg =~ /^-cow/)    { $flags .= "c"; $cowfile = shift(@args); next }
		if ($arg =~ /^-fig/) { $flags .= "f"; $figfont = shift(@args); next }
...
/COMMAND [-123456] [-blink] [-msg <target>] [-pre <prefix text>]
         [-font <figlet font>] [-cow <cowfile>] [-box] <text>
...
/jive                say something in jive
/box                 say something with a box around it
...
sub update {
	...
	my $baseURL = "http://www.gruntle.org/projects/gay";
...
Irssi::command_bind("jive", \&jivesay);
