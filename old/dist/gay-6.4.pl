...
if ($@) { 
	$can_throttle = 0;
	Irssi::print("high resolution throttling is disabled");
} else {
	$can_throttle = 1;
	Irssi::print("high resolution throttling is enabled");
}
...
sub process {
	...
	if ($flags =~ /c/ and $flags =~ /T/) {
		Irssi::print("This cow cannot THINK and SPEAK at the same time.");
		return;
	}
	...
	if ($throttle) {
		unless ($can_throttle) {
			Irssi::print("Sorry, your system does not allow high resolution sleeps");
			return;
		}

		if ($throttle < 10 or $throttle > 10_000) {
			Irssi::print("Please use a throttle between 10ms and 10,000ms");
			return;
		}
	...
	if ($flag_list =~ /(.).*\1/) {
		Irssi::print("There was an internal error with flag processing: duplicate ($1)");
		return;
	}

	foreach my $f (split(//, $flags)) {
		if ($flag_list !~ /$f/) {
			Irssi::print("There was an internal error with flag processing: missing ($f)");
			return;
		}
	}
...
sub tree {
	...
	my @output;
		...
		# generate position of bulb.. not outside edges though

		my $pos;
		if (length($line) > 2) {
			# (rnd * (max - min + 1)) + min
			my $max = length($line) - 2;
			my $min = 1;

			$pos = int(rand(0) * ($max - $min + 1)) + $min;
		}
