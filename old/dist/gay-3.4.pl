...
%IRSSI = (
	...
	changed		=> 'Sun Jul 20 20:35:18 PDT 2003',
	version		=> $VERSION,
);
...
3 = green
4 = orange
5 = red (yellow in epic/bx)
6 = magenta
7 = yellow (red in epic/bx)
8 = bright yellow
9 = bright green
10 = cyan
11 = gray
...
sub cowsay {
	...
	# this is rainbow() markup for "don't colorize until here"
...
sub rainbow {
	...
	# if this has the cowcut, don't allow
	# colorizing until it is found
	my $colorize = 1;
	if ($text =~ /$COWCUT/s) {
		$colorize = 0;
	}
	...
	foreach my $line (split(/\n/, $text)) {
		if ($colorize == 0) {
			# don't print until we get the cowcut
			if ($line eq $COWCUT) {
				$colorize = 1;
			} else {
				$newtext .= "$line\n";
