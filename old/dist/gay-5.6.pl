...
%IRSSI = (
	...
	changed		=> 'Mon Aug 25 02:13:36 PDT 2003',
	version		=> $VERSION,
);
...
/COMMAND [-123456] [-blink] [-msg <target>] [-pre <text>]
         ...
         [-spook] [-rev] [-mirror] [-leet] [-hug] <text>
EOU
...
# be in its own file.  this is stolen right out of emacs.  gg
...
sub gay {
	my $text = shift;
	if    ($text =~ /^help/i   )     { show_help()               }
	elsif ($text =~ /^vers/i   )     { Irssi::print($SPLASH)     }
	elsif ($text =~ /^update/i )     { update()                  }
	elsif ($text =~ /^usage/i  )     { show_error($USAGE)        }
	elsif ($text =~ /^col/i    )     { show_error($COLMAP)       }
...
sub reverse_ascii {
	...
	my $rnd = "TMP";	# placeholder text
	my @output;
	foreach my $line (@lines) {
		if ($line =~ /$COWCUT/) {
			push(@output, $line);
		} else {
			# swap the alignment of brackets
