...
#
# "If used sparingly, and in good taste, ASCII art generally is very well-received !"
#                             -- Some Sucker
...
%IRSSI = (
	...
	changed		=> 'Fri Jul 18 00:06:47 PDT 2003',
	version		=> $VERSION,
);
...
sub process {
	...
	# 3dbox should override box
	$flags =~ s/o//g if $flags =~ /3/;
...
sub checker {
	...
	my $check_colors = Irssi::settings_get_str('check_colors');
	unless ($check_colors =~ /^\d+\s*,\s*\d+$/) {
		Irssi::print("check_colors is invalid, please read help");
		return;
	}

	my @check_cols = split(/\s*,\s*/, $check_colors, 2);
	...
	foreach my $line (@text) {
		...
		for (my $i = 0; $i < length($line); $i = $i + $checksize) {
			...
			my $test = ($state + $offset); $test = 0 if $test > 1;

			# figure out color code
			my $code = "\x03" . $checktext . "," . $check_cols[$test] . "\26\26";

			$newline .= "$code$chunk";
			$state++; $state = 0 if $state > 1;
		}
...
sub outline {
	...
	foreach my $line (@text) {
		$line = "| $line" . (" " x ($length - clean_length($line) + 1)) . "|";
		$line .= " |" if ($_3d);
	}
